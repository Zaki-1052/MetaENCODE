...
def render_visualize_tab() -> None:
    ...
    with col1:
        ...
        # Auto-regenerate when options change (view mode or reduction method)
        if st.session_state.coords_2d is not None:
            mode_changed = (
                st.session_state.get("viz_mode") != view_mode
            )
            method_changed = (
                st.session_state.get("viz_reduction_method")
                != reduction_method
            )
            if mode_changed or method_changed:
                if view_mode == "similar_only" and similar_available:
                    generate_similar_only_visualization(
                        reduction_method, color_option
                    )
                elif view_mode == "all_datasets":
                    if not _try_load_precomputed(
                        reduction_method, filter_outliers
                    ):
